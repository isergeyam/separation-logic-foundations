...
Proof using.
  ...
  { intros ->. xapp. xsimpl. intros. reflexivity. subst. xchange* <- MList_nil. }
...
Proof using. 
  intros. gen p. induction_wf IH: list_sub L.
  xwp. xapp. xchange MList_if. xif; intros H; case_if; xpull. 
  {intros ->. xval. xsimpl. reflexivity. subst. 
  xchange* <- MList_nil. }
  intros x q L' ->. xapp. xapp. xapp. xchange <- MList_cons. 
  xsimpl. rewrite length_cons. math. 
Qed.
...
Lemma triple_acclength : forall L n c p,
...
Proof using.
  ...
  xwp. xapp. xchange MList_if. xif; intros H; case_if; xpull. 
  { intros. repeat xapp. rewrite H0. apply list_sub_cons. xsimpl. 
  rewrite H0. rewrite length_cons. math. xchange <- MList_cons. rewrite H0. xsimpl. }
  { intros. xval. xsimpl. rewrite H0. rewrite length_nil. math. 
  xchange <- MList_nil. apply C. rewrite H0. xsimpl. }
Qed.

Hint Resolve triple_acclength : triple.
...
Lemma triple_mlength' : forall L p,
  triple (mlength' p)
    ...
    (fun r => \[r = val_int (length L)] \* (MList L p)).
Proof using. 
  intros. xwp. xapp. intros. xapp. xapp. xsimpl. math. 
Qed.
...
Lemma mtriple_rev_aux: forall l2 l1 l p p1 p2,
  l = (rev l1) ++ l2 ->
  triple (mrev_aux p1 p2)
    (MList l1 p1 \* MList l2 p2)
    (funloc p' => MList (rev l) p'). 
Proof using.
  intros l2. induction_wf IH: list_sub l2. 
  xwp. xapp. xchange MList_if. xif; intros H1; case_if; xpull.
  intros. xval. xsimpl. reflexivity. rewrite H0 in H. rewrite app_nil_r in H. 
  assert (H2: rev l = rev (rev l1)). { eapply f_equal. eauto. } 
  rewrite rev_rev in H2. rewrite H2. xsimpl. intros. xapp. xapp. xapp. 
  rewrite H0. apply list_sub_cons. apply x0. rewrite H0 in H. 
  rewrite app_cons_r in H. rewrite <- rev_one in H. rewrite <- rev_app in H. 
  rewrite app_cons_one_r in H. apply H. xchange <- MList_cons. xsimpl. auto. 
Qed.

Hint Resolve mtriple_rev_aux : triple.
...
Proof using.
  intros. xwp. xchange <- MList_nil. reflexivity. xapp. xsimpl. intros. reflexivity. 
Qed.
...
Lemma triple_push : forall L s x,
  triple (push s x)
    ...
    (fun u => Stack (x::L) s).
Proof using. 
  intros. xwp. unfold Stack. xapp. intros. subst. xapp. 
  intros. repeat xapp. xsimpl. rewrite length_cons. 
  math. 
Qed.
...
Proof using. 
  intros. xwp. unfold Stack. xapp. intros. subst. destruct L. contradiction. 
  xchange MList_cons. intros. repeat xapp. xval. 
  xsimpl. reflexivity. rewrite length_cons. math. 
Qed.
...
Proof using. 
  intros. xtriple. xapp triple_mnode. intros. xchange <- MTree_Node. xsimpl*. 
Qed.
...
Proof using. 
  ...
  intros. xwp. xchange MTree_if. xapp. xif. intros H. case_if. xval. xsimpl. reflexivity. 
  intros. rewrite H0. rewrite C. xchange* <- (MTree_Leaf null). xsimpl. xchange* <- (MTree_Leaf null). xsimpl. 
  intros. case_if. xapp. intros. xapp. xapp. xapp. rewrite H0. apply tree_sub_1. 
  intros. xapp. rewrite H0. apply tree_sub_2. intros. subst. xapp. intros. 
  xsimpl*. xchange <- MTree_Node. 
Qed.
...
Lemma triple_treeacc: forall T n c p,
  triple (treeacc c p)
    (MTree T p \* c ~~> n)
    (fun _ => c ~~> (n + treesum T) \* (MTree T p)). 
Proof using.
  intros T. induction_wf IH: tree_sub T. 
  intros. xwp. xapp. xchange MTree_if. xif. intros H. case_if. xapp. 
  intros. xapp. subst. xapp. xapp. xapp. xapp. apply tree_sub_1. xapp. 
  xapp. apply tree_sub_2. xsimpl. math. xchange <- MTree_Node. 
  intros. case_if. xval. xsimpl. intros. rewrite H0. simpl. math. 
  intros. subst. xchange* <- (MTree_Leaf null). 
Qed.

Hint Resolve triple_treeacc : triple.
...
(** **** Exercise: 3 stars, standard, especially useful (triple_apply_counter_abstract)
    ...
    Hint: use [xtriple] at some point. *)
...
Proof using. 
  intros. unfold IsCounter. xtriple. xpull. intros. lets H1: H n. xapp. 
  xsimpl*. 
Qed.
