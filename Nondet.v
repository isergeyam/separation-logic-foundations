...
(** In previous chapters, we have considered a language that a
    quasi-deterministic language; only the addresses of allocated data
    could vary between two executions of a same term starting in a same state.
...
(** The judgment [evaln s t Q] can interpreted in at least five different ways:
    ...
    - The judgment [evaln] may be viewed as a generalized form of a typing
     ...
     evaln (trm_let x t1 t2) Q
      ...
      Second, let us switch from a substitution-based semantics to a
     ...
     typing E (trm_let x t1 t2) Q
      ...
      map [x] to the type [Q1]. The two presentations are thus essentially
      equivalent. *)
...
Proof using.
  intros. intros h K. applys* evalns_add. rewrite* hstar_hpure_l.
Qed.
...
Proof using.
  unfolds hoarewpn. intros. intros h K. applys* evalns_add.
Qed.
...
Proof using.
  ...
  { applys evaln_let (fun v' s' => v' = v1 /\ s' = s).
    ...
    { intros ? ? (->&->). forwards M: RS. { applys* step_let. }
      { applys M. } } }
  { applys* evalns_add. forwards M: RS. { applys* step_add. }
    { inverts* M. } }
  { applys* evaln_rand.
    { math. }
