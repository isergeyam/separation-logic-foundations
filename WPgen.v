...
(* LATER: add an example that shows how we have troubles in proofs
   when we lack integrated support for structural rules. *)
...
(** Using x-lemmas, verify the proof of [triple_succ_using_incr].
    (The proof was carried out using triples in chapter \CHAP{Rules}.) *)
...
(* LATER: AC: add an example with a recursive function *)
...
(* LATER: add an example use case for the tactic [xconseq]. *)
...
(* LATER: AC: exercise on adding support for assertions. *)
...
(** Many verification tools based on Hoare Logic are based on a weakest-
    ...
    \CITE{Hennessy and Milner 1985} on process calculi. It was first applied to
    program logic by \CITE{Honda, Berger, and Yoshida 2006}. It was then applied
    to Separation Logic in the PhD work of \CITE{Charguéraud 2010}, which
    ...
    \CITE{Guéneau, Myreen, Kumar and Norrish 2017} showed how the characteristic
